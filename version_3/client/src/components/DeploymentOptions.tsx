        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="w-16 h-16 flex items-center justify-center rounded-lg bg-white p-2 shadow-sm">
            <img 
              ...
              className="max-w-full max-h-full"
              onError={(e) => {
                // First try to load the image again with a fallback path
                const target = e.target as HTMLImageElement;
                if (target.src.includes('/logos/docker.svg')) {
                  console.log('Attempting to load docker logo from fallback path');
                  target.src = '/logos/default.svg';
                } else {
                  // If that fails too, replace with inline SVG
                  target.outerHTML = `
                    <svg width="24" height="24" class="w-10 h-10 text-blue-500" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M13 5.5H11V7.5H13V5.5Z" fill="currentColor"/>
                      <path d="M13 8.5H11V10.5H13V8.5Z" fill="currentColor"/>
                      <path d="M10 8.5H8V10.5H10V8.5Z" fill="currentColor"/>
                      <path d="M7 8.5H5V10.5H7V8.5Z" fill="currentColor"/>
